...
private[spark] object CoarseGrainedExecutorBackend extends Logging {
    ...
    def main(args: Array[String]): Unit = {
        // 创建 ExecutorBackend 的函数, 这是一个偏函数
        ...
        run(parseArguments(args, this.getClass.getCanonicalName.stripSuffix("$")), createFn) // 注意 createFn 是传递进去的一个函数
        System.exit(0)
    }
